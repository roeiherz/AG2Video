...
from evaluation.video.models.TSM.ops.basic_ops import ConsensusModule
from evaluation.video.models.TSM.ops.transforms import GroupMultiScaleCrop, GroupRandomHorizontalFlip
import numpy as np
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            ...
            if self.is_shift:
                ...
                from evaluation.video.models.TSM.ops.temporal_shift import make_temporal_shift
                make_temporal_shift(self.base_model, self.num_segments,
                                    n_div=self.shift_div, place=self.shift_place, temporal_pool=self.temporal_pool)

            if self.non_local:
                print('Adding non-local module...')
                from evaluation.video.models.TSM.ops.non_local import make_non_local
            ...
            from evaluation.video.models.TSM.archs.mobilenet_v2 import mobilenet_v2, InvertedResidual
            ...
            if self.is_shift:
                ...
                from evaluation.video.models.TSM.ops.temporal_shift import TemporalShift
            ...
            from evaluation.video.models.TSM.archs import bninception
    ...
    def forward(self, input, no_reshape=False):
        ...
        if self.reshape:
            if self.is_shift and self.temporal_pool:
                ...
                features_out = base_out.view((-1, self.num_segments // 2) + base_out.size()[1:])
            else:
                x = x.view((-1, self.num_segments) + x.size()[1:])
                features_out = base_out.view((-1, self.num_segments) + base_out.size()[1:])
            output = self.consensus(x)
            features_out = self.consensus(features_out)
            return features_out.squeeze(1), output.squeeze(1)
